...
const DB = {
  ...
  huongDanBCTTFilter: { q: '', mangDeTai: '', trangThai: '' },
  huongDanKLTNFilter: { q: '', mangDeTai: '', loaiDeTai: '', trangThai: '' },
  hdSelectedBcttDotId: null,
  phanBienFilter: { q: '', mangDeTai: '', trangThai: '' },
  hoiDongFilter: { q: '', mangDeTai: '', trangThai: '' },
  chuTichFilter: { q: '', mangDeTai: '', trangThai: '' },
  thuKyFilter: { q: '', mangDeTai: '', trangThai: '' },
};
...
function formatLateDuration(dot) {
  if (!dot || !dot.ketThuc) return '';
  const deadline = parseDateString(dot.ketThuc);
  if (!deadline) return '';
  const diffSeconds = Math.max(0, Math.floor((Date.now() - deadline.getTime()) / 1000));
  const minutes = Math.floor(diffSeconds / 60);
  const hours = Math.floor(minutes / 60);
  const days = Math.floor(hours / 24);
  if (days > 0) {
    const remainingHours = hours % 24;
    return `quá hạn ${days} ngày${remainingHours ? ` ${remainingHours} giờ` : ''}`;
  }
  if (hours > 0) {
    const remainingMinutes = minutes % 60;
    return `quá hạn ${hours} giờ${remainingMinutes ? ` ${remainingMinutes} phút` : ''}`;
  }
  if (minutes > 0) {
    return `quá hạn ${minutes} phút`;
  }
  return `quá hạn ${diffSeconds} giây`;
}
...
function toggleRevisionEditMode(recordId) {
  DB.revisionEditModeId = DB.revisionEditModeId === recordId ? null : recordId;
  refreshCurrentView();
}
...
function getKLTNAssignment(user, student) {
  ...
  return {
    ...
    isChair,
    isSecretary,
    canGrade: isAdvisor || isReviewer || isCommitteeMember || isChair,
...
function canGradeRole(user, student, vaiTro) {
  ...
  if (vaiTro === 'CT') return assignment.isChair;
...
function renderKLTNScoreBlock(k, vaiTro, title, hint, options = {}) {
  ...
  const gvhdRejected = k.tuChoiGVHD !== undefined && k.tuChoiGVHD !== null;
  const canApproveRevision = vaiTro === 'HD' && k.trangThai === 'bao_ve' && k.fileBaiChinhSua && k.fileGiaiTrinh && !gvhdApproved && !gvhdRejected;
  const gvhdApprovalStatus = vaiTro === 'HD' && k.trangThai === 'bao_ve' && k.fileBaiChinhSua && k.fileGiaiTrinh
    ? gvhdApproved
      ? `<div style="padding:10px 14px;border-radius:8px;background:#ECFDF5;border:1px solid #A7F3D0;color:#166534;font-size:13px;font-weight:600;">✅ Đã duyệt bài chỉnh sửa</div>`
      : gvhdRejected
        ? `<div style="padding:10px 14px;border-radius:8px;background:#FEE2E6;border:1px solid #FCA5A5;color:#B91C1C;font-size:13px;font-weight:600;">❌ Không duyệt bài chỉnh sửa</div>`
        : ''
    : '';
  ...
  const fileLink = (path, label) => path
    ? `<a href="${uploadFileHref(path)}" target="_blank" rel="noopener" style="color:var(--accent);font-weight:600;margin-right:12px">${label}</a>`
    : `<span style="color:var(--text3);margin-right:12px">${label} chưa có</span>`;

  const fileLinks = vaiTro === 'HD'
    ? `<div style="margin-bottom:12px;font-size:13px;color:var(--text2);">
        ${fileLink(k.fileBai, '📕 PDF')}
        ${fileLink(k.fileBaiWord, '📄 Word')}
        ${fileLink(k.fileTurnitin, '📑 Turnitin')}
        ${fileLink(k.fileBaiChinhSua, '📄 Bài chỉnh sửa')}
        ${fileLink(k.fileGiaiTrinh, '📑 Giải trình')}
        ${fileLink(k.tkBienBan, '📋 Biên bản HĐ')}
      </div>`
    : '';
  ...
  return `<div style="font-size:16px;font-weight:800;color:var(--primary-dark);margin:12px 0 8px">${escapeHtml(getScoreSheetTitle(k.topicType, vaiTro))}</div>
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      ...
      ${canApproveRevision
        ? `<button class="btn btn-success btn-sm" onclick="approveKLTNRevision('${k.id}','gvhd',true)">✅ Duyệt bài chỉnh sửa</button>
           <button class="btn btn-danger btn-sm" onclick="openRevisionRejectModal('${k.id}','gvhd')">❌ Không duyệt bài chỉnh sửa</button>`
        : gvhdApprovalStatus}
    </div>`;
}
...
function parseDateForSort(value) {
  ...
  if (isoMatch) return Date.parse(value);
  const parts = value.split(/[\/\-\.]/).map((p) => p.trim());
  if (parts.length === 3) {
    const [d, m, y] = parts;
    const year = y.length === 2 ? `20${y}` : y;
    return Date.parse(`${year.padStart(4, '0')}-${m.padStart(2, '0')}-${d.padStart(2, '0')}`);
  }
...
function getLatestBCTTBySV(email) {
  const records = getBCTTBySV(email).slice();
  if (!records.length) return null;
  records.sort((a, b) => parseDateForSort(b.ngayDangKy) - parseDateForSort(a.ngayDangKy));
  return records[0];
}
    ...
    try {
      ...
      ` + list.map(n => {
        ...
        const tieuDe = n.loai === 'tu_choi_gvhd'
          ...
          : n.loai === 'tu_choi_cthd'
            ? 'Chủ tịch HĐ không duyệt bài chỉnh sửa'
            : n.loai === 'gui_role'
              ? 'Thông báo chung'
              : 'Thông báo mới';
...
async function submitBCTT() {
  ...
  const mo_ta = (document.getElementById("f-moTa")?.value || "").trim();
  ...
  if (!ten || !cong_ty || !linh_vuc || !gv_id || !dot_id) {
  ...
  try {
    await apiRequest("/api/bctt/register", {
      method: "POST",
      body: JSON.stringify({ ten_de_tai: ten, ten_cong_ty: cong_ty, mo_ta_de_tai: mo_ta, linh_vuc, gv_id, dot_id }),
    });
...
async function submitKLTN() {
  ...
  if (loai_de_tai !== "ung_dung" && loai_de_tai !== "nghien_cuu") {
    toast("Vui lòng chọn loại đề tài KLTN", "error");
    return;
  }
  ...
  const myBCTT = getLatestBCTTBySV(DB.currentUser.email);
  if (!myBCTT || myBCTT.trangThai !== 'pass' || (myBCTT.diemBCTT !== null && myBCTT.diemBCTT < 5)) {
    toast("BCTT không đạt yêu cầu. Bạn không thể đăng ký KLTN", "error");
    return;
  }
  try {
...
function fakeUpload(kind, recordId) {
  ...
  if (isCurrentUserStudent() && isLate(dot)) {
    const lateText = formatLateDuration(dot);
    toast(`Bạn đang nộp trễ ${lateText}. Hệ thống vẫn chấp nhận file.`, "warning");
  }
...
async function submitRevision(recordId) {
  const record = findKltnRecord(recordId);
  if (!record) {
    toast("Không tìm thấy hồ sơ KLTN", "error");
    return;
  }
  if (!record.fileBaiChinhSua || !record.fileGiaiTrinh) {
    toast("Vui lòng upload đủ bài chỉnh sửa và biên bản giải trình", "error");
    return;
  }
  try {
    await apiRequest("/api/kltn/revision-submit", {
      method: "POST",
      body: JSON.stringify({ dang_ky_id: record.dangKyId || extractId(record.id) }),
    });
    toast("Đã nộp bài chỉnh sửa - chờ GVHD và Chủ tịch hội đồng duyệt");
    DB.revisionEditModeId = null;
    await refreshCurrentView();
  } catch (err) {
    toast(err.message, "error");
  }
...
function renderDashboard() {
  ...
  if (u.role === 'sv') {
    ...
    const bctt = getLatestBCTTBySV(u.email);
    const kltn = myKLTN[0];
...
function renderBCTT() {
  const u = DB.currentUser;
  const myBCTT = getBCTTBySV(u.email);
  const b = getLatestBCTTBySV(u.email);
  ...
  if (myBCTT.length > 0 && b) {
    const sortedBCTT = myBCTT.slice().sort((a, c) => parseDateForSort(c.ngayDangKy) - parseDateForSort(a.ngayDangKy));
    const historyBCTT = sortedBCTT.filter((rec) => rec.id !== b.id);
    ...
    if (historyBCTT.length > 0) {
      html += `<div class="card" style="margin-bottom:20px">
        <div class="card-header"><div><div class="card-title">🧾 Lịch sử đăng ký BCTT</div></div></div>
        <div class="table-wrapper"><table class="table">
          <thead><tr><th>Đợt</th><th>Tên đề tài</th><th>Công ty</th><th>Ngày đăng ký</th><th>Trạng thái</th><th>Điểm</th></tr></thead>
          <tbody>${historyBCTT.map((rec) => {
            const dotName = DB.dotDangKy.find(d => d.id === rec.dotId)?.ten || rec.dotId;
            return `<tr>
              <td>${escapeHtml(dotName)}</td>
              <td>${escapeHtml(rec.tenDeTai)}</td>
              <td>${escapeHtml(rec.tenCongTy || '–')}</td>
              <td>${escapeHtml(rec.ngayDangKy || '–')}</td>
              <td>${statusBadge(rec.trangThai)}</td>
              <td>${rec.diemBCTT != null && rec.diemBCTT !== '' ? escapeHtml(String(rec.diemBCTT)) : '–'}</td>
            </tr>`;
          }).join('')}</tbody>
        </table></div>
      </div>`;
    ...
    const canReRegisterBCTT = b.trangThai === 'fail' || (b.trangThai === 'pass' && b.diemBCTT != null && b.diemBCTT < 5);
    if (b.trangThai === 'pass') {
      const canKLTN = !b.diemBCTT || b.diemBCTT >= 5;
      if (canKLTN) {
        ...
        html += `<div class="card" style="background:#FFEBE6;border:1px solid #FF8F73"><div style="font-size:13px;font-weight:700;color:#BF2600">❌ BCTT không đạt yêu cầu (điểm &lt; 5). Bạn không thể đăng ký KLTN.</div></div>`;
      }
    }
    if (b.trangThai === 'fail') {
    ...
    if (canReRegisterBCTT) {
      ...
      html += `<div class="card" style="margin-top:16px">
        <div class="card-title" style="margin-bottom:20px">📋 Đăng ký lại BCTT</div>
        <div class="grid-2">
          ...
          <div class="form-group" style="grid-column:1/-1"><label>Mô tả ngắn về đề tài</label><textarea id="f-moTa" placeholder="Mô tả ngắn gọn về đề tài thực tập của bạn..." style="min-height:80px;resize:vertical"></textarea></div>
          ...
          <div class="form-group"><label>Lĩnh vực *</label><select id="f-mang" onchange="renderGVOptionsByField();"><option value="">-- Chọn lĩnh vực --</option>${fieldOptions.map((field) => `<option value="${escapeHtml(field)}">${escapeHtml(field)}</option>`).join('')}</select></div>
        ...
        <button class="btn btn-primary" style="margin-top:20px;min-width:200px" onclick="submitBCTT()">Gửi đăng ký lại BCTT</button>
      </div>`;
    }
  } else {
      const fieldOptions = getCurrentStudentFields();
...
function renderKLTN() {
  const u = DB.currentUser;
  const myKLTN = getKLTNBySV(u.email);
  const myBCTT = getLatestBCTTBySV(u.email);
  const myBCTTpass = myBCTT && myBCTT.trangThai === 'pass' && (myBCTT.diemBCTT === null || myBCTT.diemBCTT >= 5) ? myBCTT : null;
  const myBCTTfail = myBCTT && (myBCTT.trangThai === 'fail' || (myBCTT.trangThai === 'pass' && myBCTT.diemBCTT != null && myBCTT.diemBCTT < 5)) ? myBCTT : null;
  const myBCTTactive = myBCTT && ['cho_duyet', 'gv_xac_nhan', 'cho_cham'].includes(myBCTT.trangThai) ? myBCTT : null;
  ...
  if (myKLTN.length > 0) {
    ...
    const isEditingRevision = DB.revisionEditModeId === k.id;
    const hasRejection = (k.tuChoiGVHD !== undefined && k.tuChoiGVHD !== null) || (k.tuChoiCTHD !== undefined && k.tuChoiCTHD !== null);
    const revisionClosed = Boolean(k.xacNhanGVHD && k.xacNhanCTHD);
    if (hasRejection && !revisionClosed) {
      DB.revisionEditModeId = k.id; // Tự động chuyển sang chế độ chỉnh sửa khi có rejection
    }
    ...
    const hoiDongMemberNames = allHoiDongMembers
      ...
      .filter(Boolean);
    const committeeCount = hoiDongMemberNames.length;
    const committeeSummary = committeeCount
      ? `${committeeCount} thành viên${committeeCount > 3 ? ` (${hoiDongMemberNames.slice(0, 3).join(', ')}...)` : ` (${hoiDongMemberNames.join(', ')})`}`
      : 'Chưa phân công';
    const gvpbName = getUser(k.gvPBEmail)?.name || k.gvPBEmail || 'Chưa phân công';
    const chairmanName = getUser(k.hoiDong?.ct)?.name || k.hoiDong?.ct || 'Chưa phân công';
    ...
    html += `<div class="card" style="margin-bottom:20px">
      ...
      <div class="info-row"><span class="info-label">Giảng viên phản biện:</span><span class="info-value">${gvpbName}</span></div>
      <div class="info-row"><span class="info-label">Chủ tịch HĐ:</span><span class="info-value">${chairmanName}</span></div>
      <div class="info-row"><span class="info-label">Thành viên HĐ:</span><span class="info-value">${committeeSummary}</span></div>
    ...
    if (isSubmittedKLTN && !isEditingKLTN) {
      html += `<div class="card"><div class="card-title" style="margin-bottom:16px">✅ Đã nộp bài KLTN</div>
        ...
        ${isLateKLTN ? `<div style="margin-top:12px;font-size:13px;color:#b91c1c;font-weight:600">Hạn nộp KLTN đã qua. Đây là nộp trễ (${formatLateDuration(dot)}).</div>` : ''}
      ...
      html += `<div class="card"><div class="card-title" style="margin-bottom:16px">📤 Nộp bài KLTN</div>
        ...
        ${isEditingKLTN ? `<div class="grid-2">
        ...
        <button class="btn btn-secondary" style="margin-top:12px;width:100%" onclick="toggleKLTNEditMode('${k.id}')">✏️ Chỉnh sửa bài</button>` : isSubmittedKLTN ? `<button class="btn btn-secondary" style="width:100%" onclick="toggleKLTNEditMode('${k.id}')">✏️ Chỉnh sửa bài</button>` : ''}
      </div>`;
    }
    ...
    if (k.trangThai === 'bao_ve' || k.trangThai === 'hoan_thanh') {
      ...
      ${!revisionClosed ? `<div style="margin-bottom:16px;padding:12px;background:#f8f9fa;border:1px solid #e9ecef;border-radius:8px;display:flex;gap:8px;align-items:center">
        <div style="font-size:13px;font-weight:600;color:var(--text1)">Chế độ:</div>
        <button class="btn ${!isEditingRevision ? 'btn-primary' : 'btn-outline'} btn-sm" onclick="toggleRevisionEditMode('${k.id}')" ${isEditingRevision || hasRejection ? 'disabled' : ''}>📤 Nộp bài</button>
        <button class="btn ${isEditingRevision || hasRejection ? 'btn-primary' : 'btn-outline'} btn-sm" onclick="toggleRevisionEditMode('${k.id}')">✏️ Chỉnh sửa</button>
        <div style="font-size:12px;color:var(--text3);margin-left:auto">${hasRejection ? 'Có yêu cầu chỉnh sửa - cần nộp lại' : isEditingRevision ? 'Đang chỉnh sửa - có thể thay đổi file' : 'Chế độ nộp - chỉ xem file hiện tại'}</div>
      </div>` : ''}
      ...
      ? `<div style="background:#FFEBE6;border:1px solid #FF8F73;border-radius:var(--radius);padding:12px 16px;margin-bottom:12px;font-size:13px">
          ❌ <strong>Chủ tịch HĐ không duyệt bài chỉnh sửa:</strong> ${escapeHtml(k.tuChoiCTHD || 'Vui lòng xem lại nội dung báo cáo theo góp ý của hội đồng.')}
      ...
      <div>
        ...
        ${revisionClosed
          ? `<div class="upload-area" style="opacity:0.5;cursor:not-allowed;pointer-events:none">
              <div class="upload-icon">🔒</div>
              <div class="upload-text">Bài chỉnh sửa đã duyệt hoàn tất - không thể sửa</div>
            </div>`
          : `<div class="upload-area ${k.fileBaiChinhSua ? 'has-file' : ''}" onclick="${isEditingRevision ? `fakeUploadKLTN('${k.id}','fileBaiChinhSua')` : ''}" style="${!isEditingRevision ? 'opacity:0.7' : ''}">
              <div class="upload-icon">${k.fileBaiChinhSua ? '✅' : '📁'}</div>
              <div class="upload-text">${k.fileBaiChinhSua || 'Click để chọn file'}</div>
              ${!isEditingRevision ? '<div style="font-size:11px;color:#666;margin-top:4px">Chỉ xem - bật chế độ chỉnh sửa để thay đổi</div>' : ''}
            </div>`}
      </div>
      <div>
        ...
        ${revisionClosed
          ...
          : `<div class="upload-area ${k.fileGiaiTrinh ? 'has-file' : ''}" onclick="${isEditingRevision ? `fakeUploadKLTN('${k.id}','fileGiaiTrinh')` : ''}" style="${!isEditingRevision ? 'opacity:0.7' : ''}">
      ...
      <div style="margin-top:10px;font-size:12px;color:var(--text3)">Trạng thái sau bảo vệ: GVHD ${k.xacNhanGVHD ? 'Đã duyệt bài chỉnh sửa' : 'Chờ duyệt bài chỉnh sửa'} • CTHĐ ${k.xacNhanCTHD ? 'Đã duyệt bài chỉnh sửa' : k.tuChoiCTHD !== undefined && k.tuChoiCTHD !== null ? 'Không duyệt bài chỉnh sửa' : 'Chờ duyệt bài chỉnh sửa'}</div>
      ${!revisionClosed && isEditingRevision && k.fileBaiChinhSua && k.fileGiaiTrinh ? `<button class="btn btn-success" style="margin-top:16px;width:100%" onclick="submitRevision('${k.id}')">📤 Chuyển sang chế độ nộp bài</button>` : ''}
      ${!revisionClosed && !isEditingRevision && !hasRejection && k.fileBaiChinhSua && k.fileGiaiTrinh ? `<button class="btn btn-success" style="margin-top:16px;width:100%" onclick="submitRevision('${k.id}')">✅ Hoàn tất nộp bài chỉnh sửa</button>` : ''}
      </div>`;
    }
  } else {
    if (myBCTTactive) {
      ...
      html += `<div class="card" style="background:#FFEBE6;border:1px solid #FF8F73">
        <div style="font-weight:700;color:#BF2600;margin-bottom:4px">⚠️ BCTT không đạt yêu cầu</div>
        <div style="font-size:13px;color:#BF2600">BCTT không đạt điểm tối thiểu. Bạn không thể đăng ký KLTN.</div>
        <button class="btn btn-danger btn-sm" style="margin-top:12px" onclick="navigateTo('bctt')">Đăng ký lại BCTT</button>
      </div>`;
    } else if (!myBCTTpass) {
      html += `<div class="card" style="background:#FFEBE6;border:1px solid #FF8F73">
        <div style="font-weight:700;color:#BF2600;margin-bottom:4px">⚠️ Chưa hoàn tất BCTT</div>
      ...
      <div class="card">
        ...
        <div class="grid-2">
          <div class="form-group" style="grid-column:1/-1"><label>Tên đề tài *</label><input type="text" id="fk-ten" value="${myBCTTpass.tenDeTai}" placeholder="Tên đề tài KLTN..."></div>
          ...
          <div class="form-group"><label>Lĩnh vực *</label><select id="fk-mang" onchange="renderKLTNTopicTypesByField()"><option value="">-- Chọn lĩnh vực --</option>${getCurrentStudentFields().map((field) => `<option value="${escapeHtml(field)}" ${myBCTTpass.mangDeTai === field ? 'selected' : ''}>${escapeHtml(field)}</option>`).join('')}</select></div>
          <div class="form-group"><label>Loại đề tài *</label><select id="fk-topicType"><option value="">-- Chọn loại đề tài --</option><option value="ung_dung">Đề tài ứng dụng</option><option value="nghien_cuu">Đề tài nghiên cứu</option></select></div>
          <div class="form-group" style="grid-column:1/-1" ><label>GV Hướng dẫn</label><input type="text" value="${getUser(myBCTTpass.gvEmail)?.name || myBCTTpass.gvEmail}" readonly style="background:var(--bg)" id="fk-gvhd-display"><input type="hidden" id="fk-gvhd" value="${myBCTTpass.gvEmail}"></div>
        </div>
    ...
    <div class="kltn-section">
      ...
      <div class="kltn-row" style="margin-top: 15px; background: var(--bg, #fff); padding: 10px 12px; border-radius: 6px; border: 1px solid var(--border, #e5e7eb);">
        ...
        <span class="kltn-value">
          ...
          <span><b>Chủ tịch:</b> ${k.xacNhanCTHD ? '<span style="color:#16a34a; font-weight:bold;">✅ Đã duyệt bài chỉnh sửa</span>' : k.tuChoiCTHD !== undefined && k.tuChoiCTHD !== null ? '<span style="color:#ea580c; font-weight:bold;">❌ Không duyệt bài chỉnh sửa</span>' : '<span style="color:#d97706;">⏳ Chờ duyệt bài chỉnh sửa</span>'}</span>
        </span>
      </div>
      ${(k.tuChoiGVHD !== undefined && k.tuChoiGVHD !== null) || (k.tuChoiCTHD !== undefined && k.tuChoiCTHD !== null) ? `
      <div class="kltn-row" style="margin-top: 12px; background: #FEF3C7; border: 1px solid #F59E0B; border-radius: 6px; padding: 12px;">
        <span class="kltn-label" style="color: #92400E; font-weight: 600;">Lý do không duyệt:</span>
        <span class="kltn-value" style="color: #92400E;">
          ${k.tuChoiGVHD !== undefined && k.tuChoiGVHD !== null ? `<div style="margin-bottom: 6px;"><b>GVHD:</b> ${escapeHtml(k.tuChoiGVHD)}</div>` : ''}
          ${k.tuChoiCTHD !== undefined && k.tuChoiCTHD !== null ? `<div><b>Chủ tịch HĐ:</b> ${escapeHtml(k.tuChoiCTHD)}</div>` : ''}
        </span>
      </div>` : ''}
      ${u.role === 'gv' && normalizeEmail(k.gvHDEmail) === normalizeEmail(u.email) && k.trangThai === 'bao_ve' && k.fileBaiChinhSua && k.fileGiaiTrinh && !k.xacNhanGVHD && k.tuChoiGVHD == null ? `
        <div class="kltn-row" style="margin-top:12px; flex-wrap:wrap; gap:10px; align-items:center;">
          ${k.fileBaiChinhSua && k.fileGiaiTrinh
            ? `<button class="btn btn-success btn-sm" onclick="approveKLTNRevision('${k.id}','gvhd',true)">✅ Duyệt chỉnh sửa</button><button class="btn btn-danger btn-sm" onclick="openRevisionRejectModal('${k.id}','gvhd')">❌ Từ chối chỉnh sửa</button>`
            : `<span style="font-size:13px;color:var(--text3);">Sinh viên chưa nộp đủ bản sửa + biên bản giải trình để GVHD duyệt.</span>`}
        </div>
      ` : ''}
      ...
      gvList.forEach((g) => {
        ['DaiTra', 'CLC'].forEach((he) => {
          ...
          html += `<tr>
            ...
            <td>${Number(slot.slotConLai || 0)}</td>
...
async function saveGVSlotForDot(gvId, dotId, heDaoTao, slotId) {
  ...
  if (Number.isNaN(quota)) {
    toast('Quota không hợp lệ', 'error');
    return;
  }
  try {
      ...
      if (isHD) {
        ...
        html += `<div style="margin-top:12px;padding:12px 14px;border:1px solid var(--border);border-radius:var(--radius);background:var(--bg)">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            ...
            ${canReviewRevision ? `<button class="btn btn-danger btn-sm" onclick="openRevisionRejectModal('${k.id}','gvhd')">Từ chối chỉnh sửa</button>` : ''}
...
async function renderHuongDan() {
  const u = DB.currentUser;
  const bcttFilterState = DB.huongDanBCTTFilter || { q: '', mangDeTai: '', trangThai: '' };
  const kltnFilterState = DB.huongDanKLTNFilter || { q: '', mangDeTai: '', loaiDeTai: '', trangThai: '' };
  ...
  let scoredBctt = await loadScoredBCTT();
  ...
  const matchesFilter = (item, filterState) => {
    ...
    const statusValue = String(filterState.trangThai || '').trim();
    ...
    const matchesStatus = !statusValue || String(item.trangThai || '').trim() === statusValue;
    return matchesText && matchesField && matchesType && matchesStatus;
  };
  ...
  scoredBctt = scoredBctt.filter(item => matchesFilter(item, bcttFilterState));
  ...
  const bcttFieldOptions = Array.from(new Set([bcttFilterState.mangDeTai, ...bcttList, ...bcttChamList, ...scoredBctt]
    .map((item) => String(item.mangDeTai || item.loaiDeTai || '').trim())
    .filter(Boolean))).sort((a, b) => a.localeCompare(b, 'vi'));
  const bcttStatusOptions = Array.from(new Set([bcttFilterState.trangThai, ...bcttList, ...bcttChamList, ...scoredBctt]
    .map((item) => String(item.trangThai || '').trim())
    .filter(Boolean))).sort((a, b) => (STATUS_LABELS[a]?.label || a).localeCompare(STATUS_LABELS[b]?.label || b, 'vi'));
  const kltnFieldOptions = Array.from(new Set([kltnFilterState.mangDeTai, ...kltnList, ...kltnChamList]
    ...
    .filter(Boolean))).sort((a, b) => a.localeCompare(b, 'vi'));
  const kltnStatusOptions = Array.from(new Set([kltnFilterState.trangThai, ...kltnList, ...kltnChamList]
    .map((item) => String(item.trangThai || '').trim())
    .filter(Boolean))).sort((a, b) => (STATUS_LABELS[a]?.label || a).localeCompare(STATUS_LABELS[b]?.label || b, 'vi'));
  ...
  let html = `<div class="page-header"><h1>✅ Hướng dẫn</h1><p>Duyệt BCTT và KLTN theo quy trình</p></div>
    <div class="tabs">
      <button class="tab-btn ${activeTab === 'tab-bctt-huongdan' ? 'active' : ''}" onclick="switchTab(event,'tab-bctt-huongdan')">📝 BCTT (${bcttList.length + bcttChamList.length + scoredBctt.length})</button>
      ...
      <div class="card" style="margin-bottom:16px;padding:16px">
        <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:12px;align-items:flex-end">
          ...
          <div class="form-group" style="margin-bottom:0">
            ...
            <select id="huongdan-bctt-status" class="form-control" onchange="DB.huongDanBCTTFilter.trangThai=this.value; renderHuongDan();">
              <option value="">Tất cả</option>
              ${bcttStatusOptions.map((opt) => `<option value="${escapeHtml(opt)}"${opt === bcttFilterState.trangThai ? ' selected' : ''}>${escapeHtml(STATUS_LABELS[opt]?.label || opt)}</option>`).join('')}
            </select>
          </div>
        </div>
        ${bcttFilterState.q || bcttFilterState.mangDeTai || bcttFilterState.trangThai ? `<div style="margin-top:12px;color:var(--text3);font-size:13px">Đang lọc theo: ${bcttFilterState.q ? `"${escapeHtml(bcttFilterState.q)}"` : ''}${bcttFilterState.q && bcttFilterState.mangDeTai ? ' • ' : ''}${bcttFilterState.mangDeTai ? `Lĩnh vực: ${escapeHtml(bcttFilterState.mangDeTai)}` : ''}${(bcttFilterState.trangThai && (bcttFilterState.q || bcttFilterState.mangDeTai)) ? ' • ' : ''}${bcttFilterState.trangThai ? `Trạng thái: ${escapeHtml(STATUS_LABELS[bcttFilterState.trangThai]?.label || bcttFilterState.trangThai)}` : ''}</div>` : ''}
      </div>`;
  ...
  html += `<div class="card" style="margin-bottom:16px">
    <div class="card-header">
      ...
      <div style="font-size:12px;color:var(--text3)">${bcttList.length} hồ sơ</div>
    </div>`;
  if (!bcttList.length) {
  ...
  html += `</div><div id="tab-kltn-huongdan" class="tab-content ${activeTab === 'tab-kltn-huongdan' ? 'active' : ''}">
    <div class="card" style="margin-bottom:16px;padding:16px">
      <div style="display:grid;grid-template-columns:1.4fr 1fr 1fr;gap:12px;align-items:flex-end">
        ...
        <div class="form-group" style="margin-bottom:0">
          <label>Trạng thái</label>
          <select id="huongdan-kltn-status" class="form-control" onchange="DB.huongDanKLTNFilter.trangThai=this.value; renderHuongDan();">
            <option value="">Tất cả</option>
            ${kltnStatusOptions.map((opt) => `<option value="${escapeHtml(opt)}"${opt === kltnFilterState.trangThai ? ' selected' : ''}>${escapeHtml(STATUS_LABELS[opt]?.label || opt)}</option>`).join('')}
          </select>
        </div>
        <div class="form-group" style="margin-bottom:0">
      ...
      ${kltnFilterState.q || kltnFilterState.mangDeTai || kltnFilterState.loaiDeTai || kltnFilterState.trangThai ? `<div style="margin-top:12px;color:var(--text3);font-size:13px">Đang lọc theo: ${kltnFilterState.q ? `"${escapeHtml(kltnFilterState.q)}"` : ''}${(kltnFilterState.q && kltnFilterState.mangDeTai) ? ' • ' : ''}${kltnFilterState.mangDeTai ? `Lĩnh vực: ${escapeHtml(kltnFilterState.mangDeTai)}` : ''}${(kltnFilterState.trangThai && (kltnFilterState.q || kltnFilterState.mangDeTai)) ? ' • ' : ''}${kltnFilterState.trangThai ? `Trạng thái: ${escapeHtml(STATUS_LABELS[kltnFilterState.trangThai]?.label || kltnFilterState.trangThai)}` : ''}${(kltnFilterState.loaiDeTai && (kltnFilterState.q || kltnFilterState.mangDeTai || kltnFilterState.trangThai)) ? ' • ' : ''}${kltnFilterState.loaiDeTai ? `Loại đề tài: ${escapeHtml(getTopicTypeLabel(kltnFilterState.loaiDeTai))}` : ''}</div>` : ''}
    </div>`;
  ...
  html += `<div class="card">
    <div class="card-header">
      <div class="card-title">🟡 KLTN chờ duyệt</div>
      <div style="font-size:12px;color:var(--text3)">${kltnList.length} hồ sơ</div>
    </div>`;
  if (!kltnList.length) {
    html += `<div class="empty-state"><div class="empty-state-icon">✅</div><div class="empty-state-title">Không có đề tài KLTN chờ duyệt</div></div>`;
  } else {
    ...
    html += `<div class="action-row" style="margin-top:16px">
      ...
      <button class="btn btn-danger" onclick="duyetSelectedKLTNHuongDan(false)">✗ Từ chối các đề tài đã chọn</button>
    </div>`;
  }
...
function openRevisionRejectModal(recordId, step) {
  const roleLabel = step === 'cthd' ? 'Chủ tịch HĐ' : 'GVHD';
  showModal(`
    <div class="modal-card">
      <div class="modal-header">
        <div class="modal-title">${roleLabel} không duyệt bài chỉnh sửa</div>
        <button class="modal-close" onclick="closeModalForce()">✕</button>
      </div>
      <div class="modal-body" style="padding: 16px 0;">
        <div class="form-group">
          <label>Lý do không duyệt bài chỉnh sửa</label>
          <textarea id="revision-reject-reason" rows="6" style="width:100%;min-height:140px;padding:12px;border:1px solid var(--border, #d1d5db);border-radius:8px;font-size:14px;color:var(--text1, #111);" placeholder="Nhập lý do không duyệt bài chỉnh sửa..."></textarea>
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-ghost" onclick="closeModalForce()">Hủy</button>
        <button class="btn btn-primary" onclick="submitRevisionReject('${recordId}','${step}')">Gửi lý do</button>
      </div>
    </div>`);
}

function submitRevisionReject(recordId, step) {
  const ly_do = document.getElementById('revision-reject-reason')?.value.trim() || '';
  if (!ly_do) {
    return toast('Vui lòng nhập lý do không duyệt bài chỉnh sửa', 'error');
  }
  closeModalForce();
  return approveKLTNRevision(recordId, step, false, ly_do);
}

async function approveKLTNRevision(recordId, step, dongY, ly_do = '') {
  const record = findKltnRecord(recordId);
  if (!record) return toast('Không tìm thấy hồ sơ KLTN', 'error');
  if (!dongY && !ly_do) {
    openRevisionRejectModal(recordId, step);
    return;
  }
  if (!dongY && !ly_do.trim()) {
    return toast('Vui lòng nhập lý do không duyệt bài chỉnh sửa', 'error');
  ...
  try {
    ...
    if (step === 'cthd') {
      toast(dongY ? 'CTHĐ đã duyệt bài chỉnh sửa' : 'CTHĐ không duyệt bài chỉnh sửa');
    } else {
...
async function finalizeKLTNResult(recordId) {
  const record = findKltnRecord(recordId);
  if (!record) return toast('Không tìm thấy hồ sơ KLTN', 'error');
  const gvhdApproved = Boolean(record.gvhdApproved ?? record.xacNhanGVHD);
  if (!gvhdApproved) {
    return toast('GVHD cần duyệt bài chỉnh sửa trước khi kết thúc KLTN', 'error');
  }
  if (!Boolean(record.cthdApproved ?? record.xacNhanCTHD)) {
    return toast('CTHĐ cần duyệt bài chỉnh sửa trước khi kết thúc KLTN', 'error');
  }
  ...
  try {
    ...
    const data = res.data || {};
    const result = data.result === 'pass' ? 'Đạt' : data.result === 'fail' ? 'Không đạt' : (data.result || 'xong');
    toast(`Đã kết thúc KLTN và ${result}`);
    await refreshCurrentView();
  } catch (err) {
    toast(err.message, 'error');
  }
...
function renderPhanBien() {
  ...
  const filterState = DB.phanBienFilter || { q: '', mangDeTai: '', trangThai: '' };
  ...
  const matchesFilter = (item) => {
    ...
    return (!query || text.includes(query)) && (!fieldValue || String(item.mangDeTai || item.loaiDeTai || '').trim() === fieldValue)
      && (!statusValue || String(item.trangThai || '').trim() === statusValue);
  };
  ...
  const statusOptions = Array.from(new Set([statusValue, ...list.map((item) => String(item.trangThai || '').trim()).filter(Boolean)])).sort((a, b) => (STATUS_LABELS[a]?.label || a).localeCompare(STATUS_LABELS[b]?.label || b, 'vi'));
  ...
  html += `<div class="card" style="margin-bottom:16px;padding:16px">
    <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:12px;align-items:flex-end">
      ...
      <div class="form-group" style="margin-bottom:0">
        <label>Trạng thái</label>
        <select id="phanbien-status" class="form-control" onchange="DB.phanBienFilter.trangThai=this.value; renderPhanBien();">
          <option value="">Tất cả</option>
          ${statusOptions.map((opt) => `<option value="${escapeHtml(opt)}"${opt === statusValue ? ' selected' : ''}>${escapeHtml(STATUS_LABELS[opt]?.label || opt)}</option>`).join('')}
        </select>
      </div>
    </div>
  </div>
  ${filterState.q || fieldValue || statusValue ? `<div style="margin-bottom:16px;color:var(--text3);font-size:13px">Đang lọc theo: ${filterState.q ? `“${escapeHtml(filterState.q)}”` : ''}${filterState.q && fieldValue ? ' • ' : ''}${fieldValue ? `Lĩnh vực: ${escapeHtml(fieldValue)}` : ''}${((filterState.q || fieldValue) && statusValue) ? ' • ' : ''}${statusValue ? `Trạng thái: ${escapeHtml(STATUS_LABELS[statusValue]?.label || statusValue)}` : ''}</div>` : ''}`;
...
function renderHoiDong() {
  ...
  const filterState = DB.hoiDongFilter || { q: '', mangDeTai: '', trangThai: '' };
  ...
  html += `<div class="card" style="margin-bottom:16px;padding:16px">
    <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:12px;align-items:flex-end">
      ...
      <div class="form-group" style="margin-bottom:0">
        <label>Trạng thái</label>
        <select id="hoidong-status" class="form-control" onchange="DB.hoiDongFilter.trangThai=this.value; renderHoiDong();">
          <option value="">Tất cả</option>
          ${statusOptions.map((opt) => `<option value="${escapeHtml(opt)}"${opt === statusValue ? ' selected' : ''}>${escapeHtml(STATUS_LABELS[opt]?.label || opt)}</option>`).join('')}
        </select>
      </div>
...
function renderChuTich() {
  ...
  const filterState = DB.chuTichFilter || { q: '', mangDeTai: '', trangThai: '' };
  ...
  html += `<div class="card" style="margin-bottom:16px;padding:16px">
    <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:12px;align-items:flex-end">
      ...
      <div class="form-group" style="margin-bottom:0">
        <label>Trạng thái</label>
        <select id="chutich-status" class="form-control" onchange="DB.chuTichFilter.trangThai=this.value; renderChuTich();">
          <option value="">Tất cả</option>
          ${statusOptions.map((opt) => `<option value="${escapeHtml(opt)}"${opt === statusValue ? ' selected' : ''}>${escapeHtml(STATUS_LABELS[opt]?.label || opt)}</option>`).join('')}
        </select>
      </div>
  ...
  list.forEach((k) => {
    ...
    const cthdRejected = k.tuChoiCTHD !== undefined && k.tuChoiCTHD !== null;
    const cthdStatusText = cthdApproved
      ? 'Đã duyệt bài chỉnh sửa'
      : cthdRejected
        ? 'Không duyệt bài chỉnh sửa'
        : 'Chưa duyệt bài chỉnh sửa';
    const canConfirmReport = k.trangThai === 'bao_ve' && !cthdApproved && !cthdRejected;
    const canFinalize = gvhdApproved && cthdApproved;
    const finalStatusLabel = k.trangThai === 'pass'
      ? 'Đã kết thúc KLTN và Đạt'
      : k.trangThai === 'fail'
        ? 'Đã kết thúc KLTN và Không đạt'
        : null;
    html += renderKLTNRoleCardStart(k);
    html += renderKLTNScoreBlock(
      k,
      'CT',
      '👔 Chủ tịch hội đồng — phiếu chấm',
      'Chủ tịch hội đồng chấm theo mẫu phiếu của hội đồng. Điểm CT đóng góp vào điểm trung bình hội đồng.',
      { existingScore: k.diemBB, noteValue: k.ctNote || '' }
    );
    html += `<div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:12px">
      ...
      <div class="badge badge-blue">CT.HĐ: ${cthdStatusText}</div>
    </div>`;
    html += `<div style="margin-top:10px;font-size:12px;color:var(--text3)">
      ${k.trangThai !== 'bao_ve' && k.trangThai !== 'pass' && k.trangThai !== 'fail' && k.trangThai !== 'hoan_thanh'
        ...
        : cthdApproved
          ? 'Chủ tịch hội đồng đã duyệt bài chỉnh sửa cho hồ sơ này.'
          : cthdRejected
            ? 'Chủ tịch hội đồng không duyệt bài chỉnh sửa cho hồ sơ này.'
            : 'Đã đủ điều kiện để CTHĐ duyệt bài chỉnh sửa.'}
    </div>`;
    html += `<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px">
      <button class="btn btn-ghost btn-sm" onclick="viewKLTNDetail('${k.id}')">👁 Chi tiết</button>
      ${finalStatusLabel
        ? `<button class="btn btn-success btn-sm" disabled>🏁 ${finalStatusLabel}</button>`
        : `<button class="btn btn-primary btn-sm" onclick="finalizeKLTNResult('${k.id}')" ${canFinalize ? '' : 'disabled'}>🏁 Kết thúc KLTN</button>`}
      ${canConfirmReport
        ? `<button class="btn btn-success btn-sm" onclick="approveKLTNRevision('${k.id}','cthd',true)">Duyệt bài chỉnh sửa</button>
           <button class="btn btn-danger btn-sm" onclick="openRevisionRejectModal('${k.id}','cthd')">Không duyệt bài chỉnh sửa</button>`
        : cthdApproved
          ? `<div style="padding:10px 14px;border-radius:8px;background:#ECFDF5;border:1px solid #A7F3D0;color:#166534;font-size:13px;font-weight:600;">✅ Đã duyệt bài chỉnh sửa</div>`
          : cthdRejected
            ? `<div style="padding:10px 14px;border-radius:8px;background:#FEE2E6;border:1px solid #FCA5A5;color:#B91C1C;font-size:13px;font-weight:600;">❌ Không duyệt bài chỉnh sửa</div>`
...
function renderThuKy() {
  ...
  const filterState = DB.thuKyFilter || { q: '', mangDeTai: '', trangThai: '' };
  ...
  html += `<div class="card" style="margin-bottom:16px;padding:16px">
    <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:12px;align-items:flex-end">
      ...
      <div class="form-group" style="margin-bottom:0">
        <label>Trạng thái</label>
        <select id="thuky-status" class="form-control" onchange="DB.thuKyFilter.trangThai=this.value; renderThuKy();">
          <option value="">Tất cả</option>
          ${statusOptions.map((opt) => `<option value="${escapeHtml(opt)}"${opt === statusValue ? ' selected' : ''}>${escapeHtml(STATUS_LABELS[opt]?.label || opt)}</option>`).join('')}
        </select>
      </div>
...
function renderTheoDoi() {
  ...
  const canRegisterKLTN = myBCTT && myBCTT.trangThai === 'pass' && (myBCTT.diemBCTT == null || myBCTT.diemBCTT >= 5);
  const needReRegisterBCTT = myBCTT && (myBCTT.trangThai === 'fail' || (myBCTT.trangThai === 'pass' && myBCTT.diemBCTT != null && myBCTT.diemBCTT < 5));
  if (!myKLTN) {
    html += `<div class="empty-state"><div class="empty-state-icon">🎓</div><div class="empty-state-title">Bạn chưa đăng ký KLTN</div>${canRegisterKLTN ? `<button class="btn btn-primary btn-sm" style="margin-top:12px" onclick="navigateTo('kltn')">Đăng ký KLTN</button>` : needReRegisterBCTT ? `<button class="btn btn-danger btn-sm" style="margin-top:12px" onclick="navigateTo('bctt')">Đăng ký lại BCTT</button>` : ''}</div>`;
  } else {
    ...
    const steps = [
      ...
      { label: 'CTHĐ duyệt bài chỉnh sửa', done: Boolean(myKLTN.xacNhanCTHD) || ['hoan_thanh', 'pass', 'fail'].includes(st) },
...
async function duyetSelectedKLTNHuongDan(dongY) {
  const selected = Array.from(document.querySelectorAll('.select-kltn-huongdan:checked')).map(cb => parseInt(cb.value));
  if (!selected.length) {
    toast("Chưa chọn đề tài nào", "error");
    return;
  }
